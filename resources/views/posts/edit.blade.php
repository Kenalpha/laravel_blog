...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-10">
            <div class="card">
                <div class="card-header bg-primary text-white text-center">Edit Post</div>

                <div class="card-body">
                  <form method="POST" action='{{ url("editPost/{$posts->id}") }}' enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <textarea id="textarea" rows="7" class="form-control{{ $errors->has('post_body') ? ' is-invalid' : '' }}" name="post_body" value='' required>
                                  {{ $posts->post_body }}
                                </textarea>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <select id="category_id" class="form-control{{ $errors->has('category_id') ? ' is-invalid' : '' }}" name="category_id" value="{{ old('category_id') }}"  required>
                                  <option value='{{ $posts->category_id }}'>{{ $category->category }}</option>
                                  @if($categories)
